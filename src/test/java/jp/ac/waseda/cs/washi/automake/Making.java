...
public class Making {
	...
	private void makeTCs(int numberOfTC, int tcNumber) throws Exception {
		for(int i = 0; i < numberOfTC; i++){
			...
			String namesAndAddress[] = fl.makeSkeleton(tcNameLists.get(i),tcDirectory.get(tcNumber),tcPackageName.get(tcNumber),tcPackageNameforPO.get(tcNumber));
	...
	private void makePOs(int numberOfPage, int poNumber) throws Exception {
		for(int i = 0; i < numberOfPage; i++){
			...
			String nameAndAddress[] = fl.makeFile(poPageNameLists.get(i),poDirectory.get(poNumber),poPackageName.get(poNumber));
			...
			WritePage wp = new WritePage(idList,poDirectory.get(poNumber),poPageNameLists,poPageUrlLists);
