			...
			for (int idx = 0; idx < len; idx++) {
				...
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (element.getTagName().equals("TESTLISTS")) {
						...
						tcDirectory.add(child.getChildNodes().item(0)
								.getTextContent());
					} else if (element.getTagName().equals("TESTPACKAGENAME")) {
						tcPackageName.add(child.getChildNodes().item(0)
								.getTextContent());
					} else if (element.getTagName().equals("POPACKAGENAME")) {
						tcPackageNameforPO.add(child.getChildNodes().item(0)
								.getTextContent());
					} else {
						walkTreeTC(level + " ", (Element) child);
					}
			...
			for (int idx = 0; idx < len; idx++) {
				...
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (element.getTagName().equals("TEST")) {
						tcNameLists.add(child.getChildNodes().item(0)
								.getTextContent());
					} else {
						walkTreeTestList(level + " ", (Element) child);
					}
			...
			for (int idx = 0; idx < len; idx++) {
				...
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (element.getTagName().equals("PAGELISTS")) {
						...
						poDirectory.add(child.getChildNodes().item(0)
								.getTextContent());
					} else if (element.getTagName().equals("PACKAGENAME")) {
						poPackageName.add(child.getChildNodes().item(0)
								.getTextContent());
					} else {
						walkTreePO(level + " ", (Element) child);
					}
			...
			for (int idx = 0; idx < len; idx++) {
				...
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (element.getTagName().equals("NAME")) {
						poPageNameLists.add(child.getChildNodes().item(0)
								.getTextContent());
					} else if (element.getTagName().equals("URL")) {
						poPageUrlLists.add(child.getChildNodes().item(0)
								.getTextContent());
					} else {
						walkTreePage(level + " ", (Element) child);
					}
